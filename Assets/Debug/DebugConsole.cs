...
public class DebugConsole : MonoBehaviour
{
	...
	public Color assert = Color.cyan;
	...
	void Application_logMessageReceived (string condition, string stackTrace, LogType type)
	{
		...
		if(type == LogType.Error)
			colorName = "error";
		//string msgFormat = string.Format("Condition: {0} \nStack Trace: {1}", condition, stackTrace);
		DebugConsole.Log(condition, colorName);
	}
	void OnDestroy()
	{
		Application.logMessageReceived -= Application_logMessageReceived;
			...
			else
			{
				while (x < messages.Count)
				{
					...
					case "error": gui.GetComponent<GUIText>().material.color = error;
						break;
					case "assert": gui.GetComponent<GUIText>().material.color = assert;
						break;
					case "exception": gui.GetComponent<GUIText>().material.color = exception;
						break;
					}
